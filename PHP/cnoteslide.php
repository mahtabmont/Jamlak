          ...
          <div class="row content">  
              <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET">
                  <fieldset class="fsStyle">
                      <legend class="legendStyle">جستجوی آگهی ها</legend>
                      <input type="hidden" name="id" value="<?php echo $_GET["id"];?>">
               <div class="col-sm-5 col-xs-18">
                    <label class="pull-right" style="color:#800080;margin-right:20%;font-size:16px;">منطقه</label>
                    <select class="form-control" id="setmantagheid" name="m" data-live-search="true" style="padding:5px;">
				       <option  <?php if($selected_m == "خود"){ echo "selected='selected'"; } ?> value="خود">
				           منطقه خودم</option>
				       <option <?php if($selected_m == "همه"){ echo "selected='selected'"; } ?> value="همه">
				       همه مناطق</option>
				       </select>
               </div>
               
               <div class="col-sm-5 col-xs-18">
                    <label class="pull-right" style="color:#800080;margin-right:10%;font-size:16px;">آگهی های</label>
                    <select class="form-control" id="setdateid" name="t" data-live-search="true" style="padding:5px;">
				       <option <?php if($selected_t == "1"){ echo "selected='selected'"; } ?> value="1">
				           روزگذشته</option>
				       <option <?php if($selected_t == "3"){ echo "selected='selected'"; } ?> value="3">
				           سه روزگذشته</option>
				       <option <?php if($selected_t == "7"){ echo "selected='selected'"; } ?> value="7">
				           هفت روزگذشته</option>
				    </select>
               </div>
            ...
            if( strcmp($tmpmantaghe, "تهران")==0 ) 
            {
              if( (strpos("dummy" . $mahalk, "منطقه")>0) && (strlen($mahalk)<strlen("منطقه ۱۱۱")) ) //keep an eye in this for now, to improve soon en shaa Allah
              {
                $where_clause .= " || ( " . sqlcleanup("`mahal2`") . " IN 
                                       (select " . sqlcleanup("`mahal`") . " from t22mm where REPLACE(mantaghe, ' ', '') = '" . $mahalk . "')  ) ";
              }
            }
          }  
        }//for 
...
<!--------------  end of main grid- column for advertisement   ------------->
     <div class="col-sm-4">
          <div class="sidebar">
              <div id="carousel-demo" class="carousel vertical slide" data-ride="carousel" >
               <!--div id="carousel-demo" class="carousel slide" data-ride="carousel"-->
                   <ol class="carousel-indicators">
                     <?php
                     
                     $sql1 = "select * from `advertis` where `state`='".$state."' AND `paid`=1 AND `checked`=1  AND `mantaghe`='".$origmantaghe."' AND `date` >= DATE_ADD(NOW(),INTERVAL -1 Week)"; 
                     $i=0;
				     ...
				     <div class="carousel-inner">
				           ...
				           while($row1 = $result1->fetch_assoc())	{ 
                                  if ($i==0){
                                       ...
                                       echo '<div class="item">';
                                  if (trim($row1['image'])!=="")
                                       //echo '<div class="item" style="background-image: url('.'adimg/'.$row1['image'].')"></div>';
                                       echo '<img src="adimg/'.$row1['image'].'">';
                                  else
                                       //echo '<div class="item" style="background-image: url(adimg/defaultadv.jpg)"></div>';
                                       echo '<img src="adimg/defaultadv.jpg">';
                                  echo '<div class="carousel-caption">';
                                  echo '<h3>'.$row1['title'].'</h3>';
                                  echo '<p>'.$row1['tozihat'].'<br><span class="glyphicon glyphicon-phone-alt" style="color:#660066"></span><span style="color:#0077b3"><i class="glyphicon glyphicon-none"></i>'.$row1['adtel'].'</span></p>';
                                  echo '</div>';
                                  echo '</div>';
				           }
                          ?>
                   ...
                   <div class="adver text-center">
                       <p  class="active text-center" style="color:#400000;font-weight:bolder;">آگهی خود را اینجا ثبت کنید</p>
...
<div class="panel panel-primary text-right"><h5 class="panel-heading">  کاربر گرامی  هزینه چاپ آگهی شما در منطقه/شهر محل ملک برای مدت یک هفته، هزار تومان می باشد. لطفا اطلاعات زیر را تکمیل  و سپس گزینه پرداخت را انتخاب بفرمایید.</h5></div>
			...
			<p>
          	...
          	<p>
			     <label > شهر</label>
			     <select class="selectpicker" id="adcityid" name="city" data-live-search="true" > <!---->
